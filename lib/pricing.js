...
async function searchJustTCG(name, category) {
    ...
    try {
        ...
        const cards = (data.data || []).map(card => {
            ...
            // Find normal/non-foil variant, or first available
            const normalVariant = variants.find(v => v.printing === 'Normal' || v.printing === 'Non-Foil') || variants[0];
            const foilVariant = variants.find(v => v.printing === 'Holofoil' || v.printing === 'Foil' || (v.printing && v.printing.toLowerCase().includes('foil')));
            ...
            return {
                ...
                market_price: normalVariant?.price ?? null,
                foil_price: foilVariant?.price ?? null,
 ...
 * Uses Scryfall for MTG, JustTCG for TCGs.
 * Sports cards: pending eBay API integration.
 */
async function searchPricing(name, category, setName) {
    const results = [];
    const errors = [];

    // Try JustTCG (covers TCGs — not sports cards)
    const justTCGResult = await searchJustTCG(name, category);
    if (justTCGResult.results.length > 0) {
        results.push(...justTCGResult.results);
    }
    if (justTCGResult.error) {
        errors.push({ source: 'justtcg', error: justTCGResult.error });
    }
...
async function lookupInventoryPrice(item) {
    ...
    // Find best match by name + set, preferring normal printings
    const candidates = [];
    for (const r of result.results) {
        ...
        candidates.push({ ...r, nameMatch, setMatch });
    }

    // Sort: exact name+set first, then name-only, then by lowest price (avoid premium variants)
    candidates.sort((a, b) => {
        if (a.nameMatch && a.setMatch && !(b.nameMatch && b.setMatch)) return -1;
        if (b.nameMatch && b.setMatch && !(a.nameMatch && a.setMatch)) return 1;
        if (a.nameMatch && !b.nameMatch) return -1;
        if (b.nameMatch && !a.nameMatch) return 1;
        return a.market_price - b.market_price;
    });

    let bestMatch = candidates[0] || result.results[0];
