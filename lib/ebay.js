...
/**
 * Make a Trading API call (XML-based)
 */
async function tradingApiCall(accessToken, callName, xmlBody) {
    const apiUrl = isSandbox()
        ? 'https://api.sandbox.ebay.com/ws/api.dll'
        : 'https://api.ebay.com/ws/api.dll';
...
/**
 * Get seller's ended listings via Trading API
 */
async function getSellerEndedListings(accessToken) {
    const now = new Date();
...
<GetSellerListRequest xmlns="urn:ebay:apis:eBLBaseComponents">
    ...
    <ErrorLanguage>en_US</ErrorLanguage>
    <WarningLevel>High</WarningLevel>
    <EndTimeFrom>${thirtyDaysAgo.toISOString()}</EndTimeFrom>
    ...
    const text = await tradingApiCall(accessToken, 'GetSellerList', xml);

    // Parse ended items from XML response
    const items = [];
    const itemRegex = /<Item>([\s\S]*?)<\/Item>/g;
    let match;
    while ((match = itemRegex.exec(text)) !== null) {
        const itemXml = match[1];
        const itemId = itemXml.match(/<ItemID>(\d+)<\/ItemID>/)?.[1];
        const title = itemXml.match(/<Title>(.*?)<\/Title>/)?.[1];
        const listingStatus = itemXml.match(/<ListingStatus>(.*?)<\/ListingStatus>/)?.[1];

        if (itemId && listingStatus === 'Completed') {
            items.push({ itemId, title: title || '' });
        }
    }
 ...
 */
async function relistItem(accessToken, itemId) {
    const xml = `<?xml version="1.0" encoding="utf-8"?>
<RelistItemRequest xmlns="urn:ebay:apis:eBLBaseComponents">
    ...
    const text = await tradingApiCall(accessToken, 'RelistItem', xml);

    const newItemId = text.match(/<ItemID>(\d+)<\/ItemID>/)?.[1];
    const errorMatch = text.match(/<ShortMessage>(.*?)<\/ShortMessage>/);
    ...
    return { success: false, error: errorMatch ? errorMatch[1] : 'RelistItem failed' };
}
